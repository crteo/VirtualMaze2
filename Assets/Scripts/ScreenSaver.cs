...
public class ScreenSaver : BasicGUIController {
    ...
    private void Start() {
        if (Application.isEditor) { //for testing purposes.
            ChooseEyelinkFile(@"/Users/chengrenteo/Documents/20180704/180704.edf");
            ChooseSession(@"/Users/chengrenteo/Documents/20180704/shortver.txt");
            ChooseFolder(@"/Users/chengrenteo/Documents/20180704");
    ...
    public void OnRender() {
        ...
        //Console.Write("ProcessSessionDataTask Run Successfully");
        Debug.LogError(sessionInput.text);
        Console.Write("sessionInput "+ sessionInput.text);
    }
    ...
    public IEnumerator ProcessSessionDataTask(string sessionPath, string edfPath, string toFolderPath, BinMapper mapper) {
        ...
        using (RayCastRecorder recorder = new RayCastRecorder(toFolderPath, filename)) {
            yield return ProcessSession(sessionReader, eyeReader, recorder, mapper);
    ...
    private decimal EnqueueData(int numberOfTriggers, Queue<SessionData> sessionFrames, ISessionDataReader sessionReader, Queue<AllFloatData> fixations, EyeDataReader eyeReader, out int status, out string reason) {
        ...
        if (sessionData.trigger != edfTrigger) {
            Debug.Log("sessionData.trigger: " + sessionData.trigger);
            Debug.Log("edftrigger: " + edfTrigger);
            Debug.Log("Current number of triggers: " + numberOfTriggers);
    ...
    public IEnumerator ProcessSession(ISessionDataReader sessionReader, EyeDataReader eyeReader, RayCastRecorder recorder, BinMapper mapper) {
        int frameCounter = 0;
        int trialCounter = 1;
        ...
        Debug.Log("Starting ProcessSession");
        ...
        while (sessionReader.HasNext /*&& numberOfTriggers < 8*/) {
            ...
            decimal excessTime = EnqueueData(numberOfTriggers, sessionFrames, sessionReader, fixations, eyeReader, out int status, out string reason);
            ...
            while (sessionFrames.Count > 0 && fixations.Count > 0) {
                ...
                //Commented out to stop the binning
            ...
            Debug.Log("trial session: "+ trialCounter);
        }
        Console.Write("ProcessSession Ran Successfully.");
